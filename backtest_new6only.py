...
def load_fold(conn, yr):
    ...
    factors = pd.read_parquet('cache/factors_9f.parquet')
    ...
    del mcap; gc.collect()
    # 4对交互项
    for a,b in [("amihud","sr5"),("amihud","turnover_rev"),("sr5","vp_corr"),("turnover_rev","max_rev")]:
        if a in fold_df.columns and b in fold_df.columns:
            fold_df[f"{a}_x_{b}"] = fold_df[a] * fold_df[b]
...
FEATS = ['vp_corr','sr5','amihud','turnover_rev','max_rev','gap','amihud_x_sr5','amihud_x_turnover_rev','sr5_x_vp_corr','turnover_rev_x_max_rev']  # 6+4交互
...
def train_model(X, y):
    return LGBMRegressor(n_estimators=100, num_leaves=31, max_depth=5,
...
def process_final(tr, te, feat_list):
    ...
    # 贝叶斯收缩: 极端预测(>2.5σ)向中位数收缩, 防止过拟合
    pmed=te['pred'].median(); pstd=te['pred'].std()
    te.loc[te['pred']>pmed+2.5*pstd,'pred']=pmed+(te['pred']-pmed)*0.3
    te.loc[te['pred']<pmed-2.5*pstd,'pred']=pmed+(te['pred']-pmed)*0.3
