# -*- coding: utf-8 -*-
"""
Full Audit-Fixed Comparison — 分年加载版
=========================================
每个Walk-Forward fold独立加载数据 → 零内存压力 → 保留全部细节
...
用法: python backtest_full_compare.py    (全部三个配置)
      python backtest_full_compare.py A  (单个配置)
"""
...
t0 = time.time()
print('=' * 80)
print('Full Comparison Matrix: Audit-Fixed × Chunked Loading')
print('=' * 80)
...
print(f'Fee model: {MONTHLY_COST*100:.2f}%/month (stamp{STAMP*100:.2f}+comm{COMM*100:.2f}+slip{SLIP*100:.2f})')
print(f'Factors: {len(FEATS)} | Audit fixes: split-neutralize+cost+price-limit+dynamic-NLP+true-random')


def load_fold(conn, yr):
    """为单个Walk-Forward fold加载数据: train=[yr-3, yr-1], test=[yr, yr]"""
    ...
    target = conn.execute(f"""
        ...
        FROM (SELECT ts_code, trade_date, close, LEAD(close,20) OVER(PARTITION BY ts_code ORDER BY trade_date) AS fc,
                     vol FROM kline_daily
              WHERE trade_date BETWEEN '{all_start}' AND '{all_end}') s
        JOIN (SELECT trade_date, close, LEAD(close,20) OVER(ORDER BY trade_date) AS fc
              FROM kline_daily WHERE ts_code='sh000300'
                AND trade_date BETWEEN '{all_start}' AND '{all_end}') x
    ...
    fold_df['ind_name'] = fold_df['ind_name'].fillna('Other')
    fold_df = fold_df.merge(mcap, on=['ts_code','trade_date'], how='left')
    del mcap; gc.collect()
    fold_df = fold_df.merge(mkt_roll, on='trade_date', how='left')
    fold_df['mkt_sent_5d'] = fold_df['mkt_sent_5d'].fillna(0)
...
def pipeline_audited(tr, te, feat_list, use_sector_cap=True, use_conviction=True):
    """完整审计修复管线"""
    ...
    m1 = train_model(X_tr_feat, y1); m2 = train_model(X_tr_feat, y2)
    te['pred'] = (m1.predict(X_te_feat)-y1.mean())/(y1.std() or 1) + \
                 (m2.predict(X_te_feat)-0.5)/0.3

    # Fix#5: 动态NLP阈值 (用训练集分位数)
    ...
    # Fix#3: 涨停/停牌/无量过滤
    te_clean = te[
        (te['ret_1d'].fillna(0) < 0.095) &
        (te['ret_1d'].fillna(0) > -0.098) &
        (te['volume'].fillna(0) > 0)
    ].copy()
    ...
    for mo, g in te_f.groupby('ym'):
        ...
        if use_sector_cap:
            ...
            selected = [row for _, row in g.nlargest(n_base, 'pred').iterrows()]
        ...
        if use_conviction:
            ...
            gross_ret = sel_df['excess_ret_20d'].mean()
...
def run_config(conn, config_name, use_sector_cap, use_conviction):
    """跑一个完整配置的17年Walk-Forward"""
    all_rets = []; yearly = []
    for yr in range(2008, 2025):
        tr, te = load_fold(conn, yr)
        if len(tr) < 5000 or len(te) < 1000:
            print(f'  {yr}: SKIP tr={len(tr)} te={len(te)}')
            continue

        months = pipeline_audited(tr, te, FEATS, use_sector_cap, use_conviction)
        if not months:
            print(f'  {yr}: EMPTY')
            continue
    ...
    return {
        ...
        'ann_ret_gross': np.mean([m['ret_gross'] for m in all_rets])*12,
        'sharpe': np.mean(rets)/np.std(rets)*np.sqrt(12) if np.std(rets)>0 else 0,
        'mdd': np.min(np.cumprod(1+rets)/np.maximum.accumulate(np.cumprod(1+rets))-1),
...
# === 主逻辑 ===
run_only = sys.argv[1] if len(sys.argv) > 1 else None

configs = {
    'A': ('A: v1单20d+置信+行业≤5', True, True),
    'C': ('C: v1单20d+置信(无限业)', False, True),
    'D': ('D: v1单20d+等权(无限业)', False, False),
}
if run_only:
    configs = {run_only: configs[run_only]}

results = {}
for key, (label, sector, conv) in configs.items():
    print(f'\n{"="*60}')
    print(f'  {label}')
    print(f'{"="*60}')
    t_cfg = time.time()
    results[label] = run_config(con, key, sector, conv)
    print(f'  Time: {time.time()-t_cfg:.0f}s | AnnRet={results[label]["ann_ret"]*100:+.0f}% Sharpe={results[label]["sharpe"]:.2f} MDD={results[label]["mdd"]*100:.0f}%')
    gc.collect()
...
# === 最终矩阵 ===
print('\n' + '=' * 80)
print('FINAL COMPARISON MATRIX — All Audit Fixes Applied')
print('=' * 80)

print('\n  %-35s %9s %8s %8s %8s %8s' % ('Config','AnnRet','Gross','Sharpe','MDD','Late5ySh'))
print('  ' + '-'*78)
for label, r in results.items():
    print('  %-35s %+8.0f%% %+8.0f%% %7.2f %+7.0f%% %7.2f' % (
        label, r['ann_ret']*100, r['ann_ret_gross']*100, r['sharpe'], r['mdd']*100, r['late_sh']))

print('  %-35s %+8.0f%% %8s %7s %8s %7s' % ('Random (真随机全截面)', results[list(results.keys())[0]]['rand_ann']*100, '-', '-', '-', '-'))

# 分年矩阵
print('\n  %-6s' % 'Year', end='')
for label in results: print(' %9s' % label.split(':')[0], end='')
print()
for yr in range(2008, 2025):
    print('  %-6s' % yr, end='')
    for label, r in results.items():
        ydata = [y for y in r['yearly'] if y['year'] == yr]
        if ydata: print(' %+8.0f%%' % (ydata[0]['ret']*100), end='')
        else: print(' %9s' % '-', end='')
    print()
...
print(f'\n  {best_label} 年化: {best_r["ann_ret"]*100:+.0f}% (Gross: {best_r["ann_ret_gross"]*100:+.0f}%)')
print(f'  Sharpe: {best_r["sharpe"]:.2f}  MDD: {best_r["mdd"]*100:.0f}%  Late5y Sharpe: {best_r["late_sh"]:.2f}')

elapsed = time.time() - t0
print(f'\n  Total: {elapsed:.0f}s')

# Save
out = {label: {k:v for k,v in r.items() if k!='yearly'} for label, r in results.items()}
with open('cache/full_compare_results.json','w',encoding='utf-8') as f:
    json.dump(out, f, indent=2)
print('Saved: cache/full_compare_results.json')
